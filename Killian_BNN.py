...
This is adapted from the original code in the repository https://github.com/dtak/hip-mdp-public and code by Killian in the repo https://github.com/dtak/hip-mdp-public
"""
...
class BayesianNeuralNetwork(object):
	...
	def __init__(self, param_set, nonlinearity, linear_latent_weights=False):
		...
		w = 0.1* np.random.randn(self.parser.num_weights) #0.1
	...
	def __log_likelihood_factor__(self, samples_q, v_noise, X, wb, y):
		...
		if y.ndim == 1:
			return (-0.5*np.log(2*math.pi*v_noise)) - (0.5*(np.tile(np.expand_dims(y,axis=1), (self.num_weight_samples,1,1))-outputs)**2)/v_noise
		else:
	...
	def get_td_error(self, X, y, location=0.0, scale=1.0, by_dim=False,p=False):
		...
		if p:
			plt.cla()
			plt.scatter(np.arange(len(y)),outputs)
			plt.scatter(np.arange(len(y)),y)
			plt.draw()
			plt.pause(1.0/60.0)
		if y.shape[1] == 1:
			return np.sqrt((y-outputs)**2)
		else:
	...
	def fit_network(self, exp_buffer, task_weights, task_steps, state_diffs=False, use_all_exp=False):
		...
		for epoch in range(self.train_epochs):
			...
			# wb = np.array([task_weights[0,:] for tt in range(len(batch))])
			wb = np.array([task_weights[batch[tt,3],:] for tt in range(len(batch))])
			y = np.array([batch[tt,2] for tt in range(len(batch))])
			self.N = X.shape[0]
			...
			for idxs in batch_idxs:
				...
				m1_hat = m1 / (1-beta1**t)
				m2_hat = m2 / (1-beta2**t)
				# print(self.learning_rate*m1_hat/(np.sqrt(m2_hat)+epsilon))
				weights = weights - self.learning_rate*m1_hat/(np.sqrt(m2_hat)+epsilon)
			# print(self.energy(weights, X[permutation[idxs]], wb[permutation[idxs]], y[permutation[idxs]]))
